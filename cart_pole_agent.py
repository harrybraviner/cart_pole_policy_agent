...
# Define the agent
agent_state_in = tf.placeholder(shape=[None, state_size], dtype=tf.float32)
...
agent_h = tf.nn.relu(tf.matmul(agent_state_in, agent_W1) + agent_b1)
...
agent_output = tf.nn.softmax(tf.matmul(agent_h, agent_W2) + agent_b2)

agent_discounted_rewards = tf.placeholder(shape = [None], dtype = tf.float32)
agent_chosen_actions = tf.placeholder(shape = [None], dtype = tf.int32)
...
agent_update_batch = agent_optimizer.apply_gradients(zip(agent_grad_holders, agent_trainable_vars))
...
while ep < num_episodes:
    ...
    for j in range(max_ep_length):
        ...
        chosen_action = np.argmax(action_dist == chosen_action) # FIXME - I'm really not sure I get this line

        s1, r, d, _ = env.step(chosen_action)
        this_ep_history.append([state, chosen_action, r, s1])
        reward_this_ep += r
        state = s1
        ...
        if d == True: # FIXME - updates only happen when done, which we don't always reach
            ...
            discounted_rewards = discount_rewards(this_ep_history[:,2])
            feed_dict = {agent_discounted_rewards : discounted_rewards,
                         agent_chosen_actions : this_ep_history[:,1],
                         agent_state_in : np.vstack(this_ep_history[:,0])}
    ...
    if ep % 100 == 0:
        print("Trained on {} episodes".format(ep))
        print("Mean reward: {}".format(np.mean(reward_history[-100:])))
        print("Mean length: {}".format(np.mean(length_history[-100:])))
